...
def plot_csv(filenames: list[str]):
    ...
    # Left: 1 large plot spanning full height
    # Right: Individual plots stacked vertically
    num_files = len(filenames)
    fig = pyplot.figure(figsize=(14, 4 * num_files))
    gs = GridSpec(num_files, 2, figure=fig, width_ratios=[1, 1])
    
    # Create one large subplot on the left spanning all rows
    ax_combined = fig.add_subplot(gs[:, 0])
    
    # Create individual subplots on the right (one per file)
    axes_individual = [fig.add_subplot(gs[i, 1]) for i in range(num_files)]
